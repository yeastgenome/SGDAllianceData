...
from ..models.models import DBSession, Diseaseannotation, Diseasesupportingevidence, Dbentity, Straindbentity, Locussummary
from ..data_helpers.data_helpers import get_eco_ids, get_pers_output, SUBMISSION_VERSION
...
def get_disease_association_data(root_path):
    ...
    if len(list(result.keys())) > 0:
        ...
        file_name = 'src/data_dump/SGD' + SUBMISSION_VERSION + 'persist_disease_association.json'
        json_file_str = os.path.join(root_path, file_name)
        if (output_obj):
