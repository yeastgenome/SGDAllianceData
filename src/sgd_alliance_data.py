...
def get_sgdids_for_panther():
    ...
    txt_file = os.path.join(THIS_FOLDER, 'data_assets/panther_search_input.txt')
...
def get_panther_sgdids():
    ...
    json_file_str = os.path.join(THIS_FOLDER,
                             'data_assets/panther_search_results.json')

    with open(json_file_str) as json_data_file:
    ...
    if (len(combined_list) > 0):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=4) as executor:
            ...
            if (len(result) > 0):
                ...
                file_name = 'data_dump/SGD' + SUBMISSION_VERSION + 'basicGeneInformation.json'
                json_file_str = os.path.join(
                    THIS_FOLDER, file_name)
...
def get_phenotype_data():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=128) as executor:
        ...
        if len(result) > 0:
            ...
            file_name = 'data_dump/SGD' + SUBMISSION_VERSION + 'phenotype.json'
            json_file_str = os.path.join(
                    THIS_FOLDER, file_name)
...
def get_expression_data():
    ...
    desired_eco_ids = DBSession.query(Eco.eco_id).filter(
        Eco.format_name.in_(['ECO:0000314', 'ECO:0007005',
                             'ECO:0000353'])).all()
    ...
    if len(result) > 0:
        ...
        file_name = 'data_dump/SGD' + SUBMISSION_VERSION + 'expression.json'
        json_file_str = os.path.join(
                    THIS_FOLDER, file_name)
...
def run_script_group1():
    get_bgi_data()

def run_script_group2():
    get_phenotype_data()
    get_expression_data()

def run_script_without_logs():
    group1 = Thread(target=run_script_group1)
    group2 = Thread(target=run_script_group2)
    group1.start()
    group2.start()


def run_script_with_logs():
    file_name = 'logs/SGD' + SUBMISSION_VERSION + 'log_time_bgi.txt'
