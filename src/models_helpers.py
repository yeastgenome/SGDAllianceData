...
class ModelsHelper(object):
        ...
        if len(colleagues) > 0:
            ...
            for item in colleagues:
                if item.colleague_id not in colleague_obj:
                    colleague_obj[item.colleague_id] = {
                        "loci": [],
                        "relations": [],
                        "colleagues": []
                    }
                ...
                if temp_relation is not None:
                    colleague_obj[item
                                  .colleague_id]["relations"] = temp_relation
    ...
    def get_colleague_data(self, colleague, dict_data):
        ...
        if _colleague is not None:
            ...
            if _colleague:
                if len(_colleague["colleagues"]) > 0:
                    ...
                    if temp_coll:
                        ...
                        if temp_coll["address2"]:
                            if full_address:
                                full_address = full_address + ', ' + temp_coll[
                                    "address2"]
                            else:
                        ...
                        if temp_coll["other_phone"]:
                            if phones:
                                if temp_coll["work_phone"] is not temp_coll[
                                        "other_phone"]:
                                    phones = phones + ', ' + str(
                                        temp_coll["other_phone"])
                            else:
                                phones = str(temp_coll["other_phone"])
                        sfx = temp_coll[
                            "suffix"] if temp_coll["suffix"] is not None else ''
                        fname = temp_coll["first_name"] if temp_coll[
                            "first_name"] is not None else ''
                        mname = temp_coll["middle_name"] if temp_coll[
                            "middle_name"] is not None else ''
                        lname = temp_coll["last_name"] if temp_coll[
                            "last_name"] is not None else ''
                        fullname = self.modifyName(sfx, fname, mname,
                                                   lname).strip()
                ...
                locus_items = self.get_all_collegue_locus_by_id(
                    colleague.colleague_id)
                temp_locus_str = []
                if (len(locus_items) > 0):
                    temp_locus_str = [
                        lcs.locus.display_name for lcs in locus_items
                    ]
                if associates:
    ...
    def set_file_dict(self):
        file_obj = DBSession.query(
            Filedbentity, FilePath).join(FilePath).filter(
                Filedbentity.dbentity_id == FilePath.file_id,
                Filedbentity.readme_file_id != None, Filedbentity.is_public,
    ...
    def set_path_dict(self):
        path_obj = DBSession.query(Path, FilePath).join(FilePath).filter(
            Path.path_id == FilePath.path_id).all()
        obj = {}
        if path_obj:
    ...
    def get_id_list_by_path_id(self, path_id):
        file_path_ids = DBSession.query(
            FilePath.file_id).filter(FilePath.path_id == path_id).all()
    ...
    def get_files_by_id_list(self, file_id_list, flag):
        if file_id_list is not None:
            ...
            if file_res:
                ...
                for x in file_res:
                    status = ''
                    if (x.dbentity_status == "Active" or
                            x.dbentity_status == "Archived"):
